...
async function handleComparison(silent = false) {
    ...
    try {
        ...
        if (isEqual) {
            compareResultText.textContent = `MATCH: ${val1} ${unit1Label} is EQUAL to ${val2} ${unit2Label}`;
...
async function getComparisonDescription(val1, unit1, val2, unit2) {
    ...
    try {
        ...
        if (convertedValue > val2) {
            return `GREATER: ${val1} ${unit1Label} is GREATER than ${val2} ${unit2Label}`;
        } else {
            return `LESS: ${val1} ${unit1Label} is LESS than ${val2} ${unit2Label}`;
        }
    } catch (e) {
        // Fallback if conversion fails
        return `NOT EQUAL: ${val1} ${unit1Label} is NOT EQUAL to ${val2} ${unit2Label}`;
    }
}
